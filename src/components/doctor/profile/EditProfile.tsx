  ...
  useEffect(() => {
    if (user?.profile && user.userRole === 'doctor') {
      ...
      setForm({
        ...
        availableDays: Array.isArray(doctorProfile.availableDays)
          ? doctorProfile.availableDays.join(', ')
          : doctorProfile.availableDays || 'Monday - Friday',
        workingHours: doctorProfile.workingHours || '9:00 AM - 6:00 PM',
        professionalBio: doctorProfile.professionalBio || '',
        certifications: [],
        availabilityStatus: doctorProfile.availabilityStatus ?? true,
      });
    }
  }, [user]);
        ...
        <CardContent>
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                type="button"
                className={`w-10 h-6 rounded-full transition-colors ${form.availabilityStatus ? 'bg-[#8491D9]' : 'bg-gray-300 dark:bg-slate-700'}`}
                onClick={() => setForm({ ...form, availabilityStatus: !form.availabilityStatus })}
                aria-label="Toggle availability"
              >
                <span
                  className={`block w-6 h-6 rounded-full bg-white dark:bg-slate-900 shadow transform transition-transform ${form.availabilityStatus ? 'translate-x-4' : ''}`}
                />
              </button>
              <span className="text-xs text-[#8491D9]">{form.availabilityStatus ? 'Accepting new patients' : 'Not accepting'}</span>
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-[#8491D9] rounded-lg p-6 mb-2 bg-[#F5F7FD] dark:bg-slate-900 flex flex-col items-center">
                <span className="text-[#021373] dark:text-[#8491D9] font-semibold mb-2">Upload Medical Certifications</span>
                <input
                  type="file"
                  multiple
